...
import { ClassFormValues, useClassFormController } from "./addClassController";
...
const AddClassPage = ({
  ...
  const onSubmit = async () => {
    try {
      if (isEditing) {
        ...
        await controller.registerClass();
    ...
    <div className="mb-1 p-2 px-4 flex gap-4 items-center justify-between border-b border-gray-200 w-full">
      <div className="flex gap-4">
        ...
        <h2 className="m-0! text-lg font-semibold">
          {isEditing ? t("Edit Class") : t("Add Class")}
        </h2>
      </div>
      <Button
        icon={<SaveOutlined />}
  ...
  const formContent = (
    <div className={`${!isEditing ? "px-4 pb-12 overflow-y-auto h-full" : ""}`}>
      {HeaderBar}
      <AddClassForm ctrl={controller} isEditing={isEditing} />
    </div>
  );
